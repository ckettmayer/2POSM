# -*- coding: utf-8 -*-
"""
Created on Thu Nov 16 16:00:29 2023
...
#PSF para microscopio de absorción de dos fotones con un filtro de fase para tener un mínimo

#Según paper de microscopía de 2 fotones de Berland 1995 con factor donut sacado de masullo 2021
def I_donutlor(x,y,z,xs,ys,zs,I0,l,w):
    return ( (2*I0) / (np.pi*(1+ ((z-zs)**2*l**2)/(np.pi**2 *w**4) )) *2*np.e*((x**2+y**2)/(w**2)) *np.exp(- 2* ((x-xs)**2+(y-ys)**2)/(w**2*(1+((z-zs)*l/(np.pi**2 *w**2))**2)) ) )
...
I = I_donutlor(X, Y, Z,xs,ys,zs, I0, l, w)
