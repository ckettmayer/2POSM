...
ax.scatter(Phi, R, c=Iorb(R, Phi, A, np.pi/2*1, I0, w0, B), marker='o', s=5, cmap = custom_cmap.reversed(), zorder=1) 

# ax.scatter(theta, theta*0+A, c=colors, marker='o', s=100, cmap=cm, norm=norm, alpha=1)                            #orbit
ax.scatter(np.pi/2 *1, A, c=Iorb(A,np.pi/2*1,r,phi,I0,w0,B), marker='o', s=200, cmap=cm, norm=norm, alpha=1, zorder=2)      
